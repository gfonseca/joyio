...
@pytest.mark.parametrize(
    ("raw", "expected"),
    [(0, -1.0), (255, 1.0), (127, pytest.approx(-0.0039215686)), (128, pytest.approx(0.0039215686))],
)
...
def test_normalize_button_preserves_press_and_release() -> None:
    ...
    pressed_result = normalize_event(pressed)
    released_result = normalize_event(released)

    assert pressed_result is not None
    assert pressed_result.kind == "button"
    assert pressed_result.control == "BTN_SOUTH"
...
def test_normalize_absolute_axis() -> None:
    ...
    result = normalize_event(event, absinfo=AXIS_INFO)

    assert result is not None
    assert result.kind == "axis"
    assert result.control == "ABS_X"
